...
system("bcftools query -f \"\%CHROM\\t\%POS\\n\" stitch.Herato1603.3450000.3550000.PL.reheadered.vcf.gz > focal.pos");
my @calls = map{chomp; my @tmp = split "\t"; push @pos, $tmp[0]; [ @tmp[1..$#tmp] ]} `bcftools query -f "%POS[\t%GT]\n" stitch.Herato1603.3450000.3550000.PL.reheadered.vcf.gz`;
...
my @glob_matches;
my @glob_comps;
open (OUT, ">stitch.Herato1603.3450000.3550000.PL.GTmatch.out");
my @seg_size;
